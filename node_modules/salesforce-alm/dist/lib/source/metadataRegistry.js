...
const describeMetadataDecompositionConfig_1 = require("./decompositionStrategy/describeMetadataDecompositionConfig");
const decompositionConfigFactory_1 = require("./decompositionStrategy/decompositionConfigFactory");
const typeDefObj_1 = require("./typeDefObj");
// Constants
...
const _functionBundleTypes = {
    // More files to come
    METADATA: {
        fileSuffix: '.code' // -meta.xml
    }
};
...
const _createMetadataInfos = function (metadataDescribeResults) {
    ...
    for (let i = 0; i < metadataObjects.length; i++) {
        ...
        if (!BlacklistManager.isBlacklisted(metadataName)) {
            ...
            const typeDefObject = new typeDefObj_1.TypeDefObj(metadataName);
            ...
            if (info.inFolder) {
                const folderMetadataName = hasOverride && typeDefOverride.nameForFolder !== undefined
                    ? `${typeDefOverride.nameForFolder}Folder`
                    : `${metadataName}Folder`;
                const folderTypeDefObject = new typeDefObj_1.TypeDefObj(folderMetadataName);
                ...
                folderTypeDefObject.decompositionConfig = decompositionConfigFactory_1.DecompositionConfigFactory.getDecompositionConfig(folderTypeDefObject);
            }
            typeDefObject.decompositionConfig = decompositionConfigFactory_1.DecompositionConfigFactory.getDecompositionConfig(typeDefObject);
        }
    }
...
class MetadataRegistry {
    constructor(org) {
        ...
        this.functionBundleTypes = _functionBundleTypes;
    ...
    getTypeDefinitionByFileName(filePath, useTrueExtType) {
        ...
        if (workspaceFilePath.includes(`${path.sep}${this.typeDefs.FunctionBundle.defaultDirectory}${path.sep}`)) {
            return this.typeDefs.FunctionBundle;
        }
    ...
    isValidFunctionBundleSuffix(suffix) {
        return this.functionBundleTypes;
    }
    ...
    isValidSourceFilePath(sourcePath) {
        ...
        else if (sourcePath.includes(`${path.sep}functionBundles${path.sep}`)) {
            const suffix = '.' + fileName.split('.').pop();
            return this.isValidFunctionBundleSuffix(suffix);
        }
