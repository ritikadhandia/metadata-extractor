 ...
 * @private
 */
const _updateFromPath = function (sourcePath, workspacePathInfos, forceIgnore, isWorkspace, isArtifactRoot, metadataRegistry) {
...
const _readSourcePathInfos = function (org) {
    ...
    try {
        const oldSourcePathInfos = new Map(org.getSourcePathInfos().read());
        ...
        for (const sourcePathInfoObj of oldSourcePathInfos.values()) {
            ...
            if (workspacePathChanged) {
                sourcePathInfo.sourcePath = path.join(workspacePath, path.relative(oldWorkspacePath, sourcePathInfo.sourcePath));
            }
...
function SourcePathStatusManager(org, isStateless) {
    ...
    if (!isStateless) {
        ...
        if (_.isNil(this.workspacePathInfos)) {
            ...
            [this.workspacePathInfos, this.artifactRootPaths] = _initializeWorkspace(this.org, this.forceIgnore, this.metadataRegistry);
        }
    }
    else {
 ...
 */
SourcePathStatusManager.prototype.getSourcePathInfos = function (filter = {}) {
    ...
    const untrackedArtifactRootPaths = currentArtifactRootPaths.filter(rootDir => !oldArtifactRootPaths.includes(rootDir));
    // if a root directory is specified, make sure it is a project source directory
    if (!_.isNil(packageDirectory) &&
        ...
        if (_.isNil(pendingSourcePathInfo)) {
            ...
            if (shouldIncludeSourcePathInfo) {
                ...
                if (pendingSourcePathInfo.isDirectory &&
                    ...
                    sourcePathInfos = sourcePathInfos.concat(_processChangedDirectory(pendingSourcePathInfo.sourcePath, this.workspacePathInfos, this.forceIgnore, this.metadataRegistry));
                }
            }
        }
    }
...
module.exports = SourcePathStatusManager;
