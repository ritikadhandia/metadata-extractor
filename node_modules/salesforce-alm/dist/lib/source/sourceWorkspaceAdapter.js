...
function _SourceLocations(sourceFileInfos, metadataRegistry) {
    ...
    for (const sourceFileInfo of sourceFileInfos) {
        if (sourceFileInfo.isMetadataFile) {
            const pathMetadataType = metadataTypeFactory_1.MetadataTypeFactory.getMetadataTypeFromSourcePath(sourceFileInfo.sourcePath, metadataRegistry);
            if (pathMetadataType) {
                ...
                if (kit_1.isEmpty(sourceFileInfo.sourcePath) || !ts_types_1.isString(sourceFileInfo.sourcePath)) {
                    throw new Error(`Invalid source path for metadataType: ${pathMetadataType}`);
                }
                else {
                    const aggregateMetadataPath = pathMetadataType.getAggregateMetadataFilePathFromWorkspacePath(sourceFileInfo.sourcePath);
...
class SourceWorkspaceAdapter extends kit_1.AsyncCreatable {
     ...
     */
    getAggregateSourceElements(changesOnly, packageDirectory, updatePendingPathInfos = false, sourcePath) {
     ...
     */
    updateSource(sourceElements, force, sOrgApi, manifest, checkForDuplicates, unsupportedMimeTypes) {
        let updatedPaths = [];
        let deletedPaths = [];
