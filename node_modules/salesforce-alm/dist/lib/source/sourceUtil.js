 ...
 */
exports.getSourceElementForFile = function (sourcePath, sourceWorkspaceAdapter, metadataType) {
    ...
    if (sourceElementMetadataType) {
        ...
        aggregateSourceElement = exports.loadSourceElement(sourceElements, key, mdRegistry);
    }
    else {
...
exports.getSourceElementsFromSourcePath = async function (optionsSourcePath, sourceWorkspaceAdapter) {
    let aggregateSourceElements = new Map();
    const mdRegistry = sourceWorkspaceAdapter.metadataRegistry;
    for (let sourcepath of optionsSourcePath.split(',')) {
        ...
        if (srcDevUtil.containsFileExt(sourcepath)) {
            const ase = exports.getSourceElementForFile(sourcepath, sourceWorkspaceAdapter, metadataType);
            ...
            aggregateSourceElements = new Map([
                ...
                ...exports.getSourceElementsInPath(sourcepath, sourceWorkspaceAdapter)
            ]);
        }
    }
 ...
 * @returns {AggregateSourceElement}
 */
exports.loadSourceElement = function (sourceElements, key, metadataRegistry) {
    let aggregateSourceElement = sourceElements.get(key);
    if (!aggregateSourceElement) {
        ...
        if (hasParentType) {
            ...
            return exports.loadSourceElement(sourceElements, newKey, metadataRegistry);
        }
            ...
            return exports.loadSourceElement(sourceElements, `${mdType}__${mdName}`, metadataRegistry);
        }
            ...
            return exports.loadSourceElement(sourceElements, name, metadataRegistry);
        }
        else {
 ...
 */
exports.updateMaxRevision = async function (org, metadataRegistry, members = []) {
...
exports.getMemberNamesFromPushResult = async function (metadataRegistry, pushResult) {
    const smmHelper = new SourceMetadataMemberRetrieveHelper(metadataRegistry);
    const allMembers = await smmHelper.getAllMemberNames();
    const successes = ts_types_1.get(pushResult, 'details.componentSuccesses', []);
 ...
 */
function getRevisionFieldName(config = new configApi_1.Config(), env = new kit_1.Env()) {
...
exports.getRevisionFieldName = getRevisionFieldName;
var RevisionCounterField;
(function (RevisionCounterField) {
    RevisionCounterField["RevisionCounter"] = "RevisionCounter";
    RevisionCounterField["RevisionNum"] = "RevisionNum";
})(RevisionCounterField = exports.RevisionCounterField || (exports.RevisionCounterField = {}));
