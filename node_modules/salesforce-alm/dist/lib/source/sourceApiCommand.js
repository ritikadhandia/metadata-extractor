...
class SourceApiCommand {
    ...
    async execute(context) {
        ...
        return MetadataRegistry.initializeMetadataTypeInfos(this.orgApi)
            .then(() => this.isDeploy() ? sourceDeployApi_1.SourceDeployApi.create({ org: this.orgApi }) : sourcePushApi_1.MdapiPushApi.create({ org: this.orgApi }))
            .then((deployApi) => {
            ...
            if (e.name === 'SourceConflict') {
                ...
                const error = new core_1.SfdxError(messageBundle.getMessage(`source${this.deploytype}Failed`), 'DeployFailed');
            ...
            if (result.userCanceled) {
                ...
                result.outboundFiles.forEach((sourceElement) => syncCommandHelper.createDisplayRows(rows, sourceElement, projectPath));
            }
        })
            .then(() => {
            if (this.isAsync) {
