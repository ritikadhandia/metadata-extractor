...
class PackageVersionCreateCommand {
        ...
        xml2js.parseStringAsync(currentPackageXml))
            ...
            if (metadataExclusions && metadataExclusions.length > 0) {
                typesArr = packageJson.Package.types.map(element => {
                    ...
                    obj.members = element.members.filter(member => _.isNil(metadataExclusions.find(exclusion => exclusion.type === element.name[0] && exclusion.name === member)));
                    return obj;
                });
            }
        ...
        return this.packageVersionCreateRequestApi.byId(id).then(async (results) => {
            if (this._isStatusEqualTo(results, [STATUS_SUCCESS, STATUS_ERROR])) {
                ...
                if (retries > 0) {
                    ...
                    logger.log(`Request in progress. Sleeping ${this.pollInterval} seconds. Will wait a total of ${this.pollInterval *
                        retries} more seconds before timing out. Current Status='${pkgUtils.convertCamelCaseStringToSentence(currentStatus)}'`);
                    return BBPromise.delay(this.pollInterval * 1000).then(() => this._pollForStatus(context, id, retries - 1, packageId));
                }
                else {
                    // Timed out
                }
    ...
    async _execute(context) {
        ...
        return configContentPromise.then(async (configContent) => {
            ...
            return MetadataRegistry.initializeMetadataTypeInfos(this.org).then(() => this._createPackageVersionCreateRequestFromOptions(context, resolvedPackageId)
